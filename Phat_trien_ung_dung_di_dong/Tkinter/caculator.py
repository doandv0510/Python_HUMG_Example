...
display = Entry(window, font=('Tacoma', 20), width=30, justify='right')
...
for i in range(len(buttons)):
    ...
    for j in range(len(line)):
        ch = line[j]
        btn = Button(window, text=ch, font=('Tacoma', 20), command=on_btn_clicked(ch))
        btn.grid(row=i + 1, column=j, sticky='news', padx=5, pady=5)
window.mainloop()
